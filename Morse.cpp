...
void Morse::write(char val) {
  val = toupper(val);
  if ( val < 0x20 || val > 0x5f ) return;
  
  MorseCode_t morse;
  byte *ptr = (byte *)&morse;
  for ( int i=0; i<sizeof(MorseCode_t); ++i )
    *ptr = pgm_read_byte_near(&gMorseCode[val - 0x20] + i);
  
  // debug output (to check pgm_read_byte_near() operation...)
  Serial.print((int)morse.code);
  Serial.print(" ");
